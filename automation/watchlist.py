"""공개 대시보드용 상세분석 대상 종목. DART/pykrx 호출량을 감당할 수 있는 범위로 시가총액
상위 우량주 위주로 큐레이션했다(라이브로 "실시간 시가총액 순위"를 매길 방법이 KRX 전종목
조회 차단 때문에 막혀 있어 — automation/generate_attention.py와 동일한 제약 — 수작업으로 구성).

전체 상장사(코스피+코스닥 약 2,500개+) 검색은 이 목록과 무관하게 generate_company_index.py가
DART corpCode 전체를 인덱싱해서 지원한다. 이 목록에 없는 종목은 검색은 되지만 상세분석
(기술적/기본적분석 차트·점수)은 제공되지 않는다 — 프론트엔드가 이를 명시적으로 안내한다.

종목을 추가/제거하려면 이 리스트만 수정하면 된다(다른 코드 변경 불필요). 코드가 틀렸거나
상장폐지된 종목은 파이프라인이 개별적으로 스킵+로그 처리하므로 전체를 막지 않는다.
"""

WATCHLIST = [
    # 반도체/전자부품
    {"code": "005930", "name": "삼성전자", "sector": "반도체"},
    {"code": "000660", "name": "SK하이닉스", "sector": "반도체"},
    {"code": "042700", "name": "한미반도체", "sector": "반도체"},
    {"code": "009150", "name": "삼성전기", "sector": "전자부품"},
    {"code": "000990", "name": "DB하이텍", "sector": "반도체"},
    {"code": "058470", "name": "리노공업", "sector": "반도체"},
    {"code": "240810", "name": "원익IPS", "sector": "반도체장비"},
    {"code": "036930", "name": "주성엔지니어링", "sector": "반도체장비"},
    {"code": "222800", "name": "심텍", "sector": "전자부품"},
    {"code": "095340", "name": "ISC", "sector": "반도체"},
    {"code": "357780", "name": "솔브레인", "sector": "반도체소재"},
    {"code": "005290", "name": "동진쎄미켐", "sector": "반도체소재"},
    {"code": "064760", "name": "티씨케이", "sector": "반도체소재"},
    {"code": "034220", "name": "LG디스플레이", "sector": "디스플레이"},

    # 전자제품
    {"code": "066570", "name": "LG전자", "sector": "전자제품"},

    # 2차전지
    {"code": "373220", "name": "LG에너지솔루션", "sector": "2차전지"},
    {"code": "006400", "name": "삼성SDI", "sector": "2차전지"},
    {"code": "003670", "name": "포스코퓨처엠", "sector": "2차전지소재"},
    {"code": "247540", "name": "에코프로비엠", "sector": "2차전지소재"},
    {"code": "086520", "name": "에코프로", "sector": "2차전지소재"},
    {"code": "066970", "name": "엘앤에프", "sector": "2차전지소재"},
    {"code": "005070", "name": "코스모신소재", "sector": "2차전지소재"},
    {"code": "361610", "name": "SK아이이테크놀로지", "sector": "2차전지소재"},

    # 자동차/부품
    {"code": "005380", "name": "현대차", "sector": "자동차"},
    {"code": "000270", "name": "기아", "sector": "자동차"},
    {"code": "012330", "name": "현대모비스", "sector": "자동차부품"},
    {"code": "011210", "name": "현대위아", "sector": "자동차부품"},
    {"code": "018880", "name": "한온시스템", "sector": "자동차부품"},
    {"code": "204320", "name": "HL만도", "sector": "자동차부품"},
    {"code": "161390", "name": "한국타이어앤테크놀로지", "sector": "자동차부품"},
    {"code": "002350", "name": "넥센타이어", "sector": "자동차부품"},

    # 철강/화학/소재
    {"code": "005490", "name": "POSCO홀딩스", "sector": "철강"},
    {"code": "004020", "name": "현대제철", "sector": "철강"},
    {"code": "010130", "name": "고려아연", "sector": "비철금속"},
    {"code": "051910", "name": "LG화학", "sector": "화학"},
    {"code": "011170", "name": "롯데케미칼", "sector": "화학"},
    {"code": "011780", "name": "금호석유화학", "sector": "화학"},
    {"code": "009830", "name": "한화솔루션", "sector": "화학"},
    {"code": "010060", "name": "OCI홀딩스", "sector": "화학"},
    {"code": "298050", "name": "효성첨단소재", "sector": "화학소재"},
    {"code": "120110", "name": "코오롱인더스트리", "sector": "화학소재"},
    {"code": "006650", "name": "대한유화", "sector": "화학"},
    {"code": "010950", "name": "S-Oil", "sector": "정유"},

    # 인터넷/게임/미디어/엔터
    {"code": "035420", "name": "NAVER", "sector": "인터넷"},
    {"code": "035720", "name": "카카오", "sector": "인터넷"},
    {"code": "259960", "name": "크래프톤", "sector": "게임"},
    {"code": "036570", "name": "NC", "sector": "게임"},
    {"code": "251270", "name": "넷마블", "sector": "게임"},
    {"code": "293490", "name": "카카오게임즈", "sector": "게임"},
    {"code": "263750", "name": "펄어비스", "sector": "게임"},
    {"code": "112040", "name": "위메이드", "sector": "게임"},
    {"code": "253450", "name": "스튜디오드래곤", "sector": "미디어"},
    {"code": "035760", "name": "CJ ENM", "sector": "미디어"},
    {"code": "352820", "name": "하이브", "sector": "엔터"},
    {"code": "035900", "name": "JYP Ent.", "sector": "엔터"},
    {"code": "041510", "name": "에스엠", "sector": "엔터"},
    {"code": "122870", "name": "와이지엔터테인먼트", "sector": "엔터"},

    # 바이오/제약
    {"code": "207940", "name": "삼성바이오로직스", "sector": "바이오"},
    {"code": "068270", "name": "셀트리온", "sector": "바이오"},
    {"code": "302440", "name": "SK바이오사이언스", "sector": "바이오"},
    {"code": "000100", "name": "유한양행", "sector": "제약"},
    {"code": "128940", "name": "한미약품", "sector": "제약"},
    {"code": "069620", "name": "대웅제약", "sector": "제약"},
    {"code": "185750", "name": "종근당", "sector": "제약"},
    {"code": "006280", "name": "녹십자", "sector": "제약"},
    {"code": "196170", "name": "알테오젠", "sector": "바이오"},
    {"code": "214150", "name": "클래시스", "sector": "의료기기"},
    {"code": "145720", "name": "덴티움", "sector": "의료기기"},

    # 금융
    {"code": "105560", "name": "KB금융", "sector": "금융"},
    {"code": "055550", "name": "신한지주", "sector": "금융"},
    {"code": "086790", "name": "하나금융지주", "sector": "금융"},
    {"code": "316140", "name": "우리금융지주", "sector": "금융"},
    {"code": "138930", "name": "BNK금융지주", "sector": "금융"},
    {"code": "175330", "name": "JB금융지주", "sector": "금융"},
    {"code": "032830", "name": "삼성생명", "sector": "보험"},
    {"code": "000810", "name": "삼성화재", "sector": "보험"},
    {"code": "005830", "name": "DB손해보험", "sector": "보험"},
    {"code": "138040", "name": "메리츠금융지주", "sector": "금융"},
    {"code": "071050", "name": "한국금융지주", "sector": "증권"},
    {"code": "005940", "name": "NH투자증권", "sector": "증권"},
    {"code": "006800", "name": "미래에셋증권", "sector": "증권"},
    {"code": "016360", "name": "삼성증권", "sector": "증권"},
    {"code": "039490", "name": "키움증권", "sector": "증권"},
    {"code": "323410", "name": "카카오뱅크", "sector": "금융"},

    # 지주/상사
    {"code": "028260", "name": "삼성물산", "sector": "지주/상사"},
    {"code": "034730", "name": "SK", "sector": "지주/상사"},
    {"code": "003550", "name": "LG", "sector": "지주/상사"},
    {"code": "004990", "name": "롯데지주", "sector": "지주/상사"},
    {"code": "267250", "name": "HD현대", "sector": "지주/상사"},

    # 조선/중공업
    {"code": "009540", "name": "HD한국조선해양", "sector": "조선"},
    {"code": "010140", "name": "삼성중공업", "sector": "조선"},
    {"code": "042660", "name": "한화오션", "sector": "조선"},
    {"code": "329180", "name": "HD현대중공업", "sector": "조선"},
    {"code": "034020", "name": "두산에너빌리티", "sector": "중공업"},
    {"code": "010620", "name": "HD현대미포", "sector": "조선"},

    # 항공/운송
    {"code": "003490", "name": "대한항공", "sector": "항공"},
    {"code": "180640", "name": "한진칼", "sector": "항공"},
    {"code": "011200", "name": "HMM", "sector": "해운"},

    # 식품/유통
    {"code": "097950", "name": "CJ제일제당", "sector": "식품"},
    {"code": "271560", "name": "오리온", "sector": "식품"},
    {"code": "004370", "name": "농심", "sector": "식품"},
    {"code": "000080", "name": "하이트진로", "sector": "식품"},
    {"code": "023530", "name": "롯데쇼핑", "sector": "유통"},
    {"code": "139480", "name": "이마트", "sector": "유통"},
    {"code": "007070", "name": "GS리테일", "sector": "유통"},
    {"code": "282330", "name": "BGF리테일", "sector": "유통"},

    # 건설
    {"code": "000720", "name": "현대건설", "sector": "건설"},
    {"code": "006360", "name": "GS건설", "sector": "건설"},
    {"code": "047040", "name": "대우건설", "sector": "건설"},
    {"code": "375500", "name": "DL이앤씨", "sector": "건설"},

    # 통신
    {"code": "017670", "name": "SK텔레콤", "sector": "통신"},
    {"code": "030200", "name": "KT", "sector": "통신"},
    {"code": "032640", "name": "LG유플러스", "sector": "통신"},

    # 방산/항공우주
    {"code": "012450", "name": "한화에어로스페이스", "sector": "방산"},
    {"code": "079550", "name": "LIG디펜스앤에어로스페이스", "sector": "방산"},
    {"code": "047810", "name": "한국항공우주", "sector": "방산"},

    # 생활용품/화장품/기타 소비재
    {"code": "090430", "name": "아모레퍼시픽", "sector": "화장품"},
    {"code": "051900", "name": "LG생활건강", "sector": "화장품"},
    {"code": "004170", "name": "신세계", "sector": "유통"},
    {"code": "033780", "name": "KT&G", "sector": "소비재"},
]

WATCHLIST_BY_CODE = {row["code"]: row for row in WATCHLIST}
