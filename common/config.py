...
TTL_NO_STATEMENT = 5 * 24 * 3600  # 분기 재무제표 없음 확인: 5일(분기공시는 자주 바뀌어 짧게)
# 연간(사업보고서) 무재무 음성캐시는 더 길게 둔다 — 스팩·리츠·상폐 등 앞으로도 연간이 없는 종목이
# 대부분이라, 짧은 TTL로 5일마다 전부 재탐색하면 그 헛조회가 DART 일일한도를 잠식해 정작 필요한
# 워밍을 늦춘다. 연간 사업보고서는 1년에 한 번(2~3월) 제출되므로 신규 상장주는 최대 3주 내 반영되면
# 충분하다(연간 지표는 연 1회 갱신이라 지연이 무의미).
TTL_NO_ANNUAL = 21 * 24 * 3600  # 연간 재무제표 없음 확인: 3주
