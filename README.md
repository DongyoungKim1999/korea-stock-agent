...
- **상세분석 4축**: 큐레이션된 우량주 120종목(`automation/watchlist.py`)에 대해 아래 네 관점을 한
  화면에 제공한다. 목록 밖 종목을 선택하면 "상세분석 미지원"으로 안내(종목명·코드까지는 확인 가능):
  - **기술적분석**: 이동평균·MACD·RSI·볼린저·거래량·캔들 종합 1~5점
  - **기본적분석**: 안정성/성장성/활동성 업종평균 대비 상대점수
  - **밸류에이션**(신규): PER(연환산)·PBR — "지금 이 가격이 싼가/비싼가"
  - **재무건전성**(신규): Piotroski F-Score(0~9)·ROE·ROA — "재무가 튼튼하고 개선 중인가"
  - **리스크**(신규): 연변동성·최대낙폭(MDD)·52주 위치 — "얼마나 출렁이고 물리면 얼마나 깨지나"
